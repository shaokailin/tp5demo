...
#define PRODUCTID @"495100" //商品ID（请填写你商品的id）
#define PRODUCT99 @"9910400" //商品ID（请填写你商品的id）


@interface GoldTwoView ()
...
- (void)JBladeData{
    ...
    [HttpUtils postRequestWithURL:URL11 withParameters:@{@"uid":uidS} withResult:^(id result) {
        ...
        NSString *st1 = [NSString stringWithFormat:@"%@秀币",_dataDic[@"recharge"][0][@"gold"]];
        self.dayLabel1.text = st1;
        NSString *Mst1 = [NSString stringWithFormat:@"￥ %@元",_dataDic[@"recharge"][0][@"money"]];
        self.monyLabel1.text = Mst1;
        
        NSString *st2 = [NSString stringWithFormat:@"%@秀币",_dataDic[@"recharge"][1][@"gold"]];
        self.dayLabel2.text = st2;
        NSString *Mst2 = [NSString stringWithFormat:@"￥ %@元",_dataDic[@"recharge"][1][@"money"]];
...
- (void)ZFdata{
    ...
    NSString *accountId = [NSString stringWithFormat:@"%@",self.dataDic[@"recharge"][HH][@"id"]];
    
    NSDictionary *dic = @{@"uid":uidS,@"accountType":@"2",@"accountId":accountId,@"phoneType":@"2",@"accountForm":fangshi,@"hash":@""};
...
- (void)testPay {
    ...
    if ([SKPaymentQueue canMakePayments]) {
        if ([self.motype isEqualToString:@"0"]) {
            ...
            [self requestProductData:PRODUCT99];
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response{
    ...
    for (SKProduct *pro in product) {
        ...
        if ([self.motype isEqualToString:@"0"]) {
            ...
            if([pro.productIdentifier isEqualToString:PRODUCT99]){
                p = pro;
            }
        }
    }
