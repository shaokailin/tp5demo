...
@interface SiimaViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>
{
    NSInteger page;
}
...
@implementation SiimaViewController
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (SEE == 0) {
        ...
        //添加到要有毛玻璃特效的控件中
        effectView.frame = CGRectMake(0, 0, kScreen_w /3 - 5, kScreen_w /3 - 5);
        [pictCell.imageView addSubview:effectView];
        //设置模糊透明度
        effectView.alpha = 0.99;
        pictCell.imaButton.alpha = 1;
        [pictCell.imaButton setBackgroundImage:[UIImage imageNamed:@"icon-lock-"] forState:UIControlStateNormal];
    }else {
        [pictCell.imageView sd_setImageWithURL:[NSURL URLWithString:model.uploadfiles] placeholderImage:[UIImage imageNamed:zhanImage]];
...
- (void)addData{
    ...
    NSDictionary *dic= @{@"fromuid":uidS,@"touid":self.UID,@"photoType":@"1"};
