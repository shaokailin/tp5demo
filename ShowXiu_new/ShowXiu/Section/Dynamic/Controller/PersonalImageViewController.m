...
@interface PersonalImageViewController ()<UITableViewDelegate, UITableViewDataSource>
...
//                cell.oneImagView.hidden = 1;
//                cell.twoImageView.hidden = 1;
//                cell.threeImageView.hidden = 1;
//                cell.fourImageView.hidden = 1;
            }else  {
...
//    NSDictionary *dic1 = _model.thumbfiles[0];
    NSUserDefaults *defatults = [NSUserDefaults standardUserDefaults];
    NSDictionary *moDia = self.photoidArray[_HHKK];
    self.photoid = moDia[@"photoid"];
    NSString *uidS = [defatults objectForKey:uidSG];
    NSString *URL = [NSString stringWithFormat:@"%@%@",AppURL,API_photohead];
    NSDictionary *dic = @{@"uid":uidS,@"photoid":moDia[@"photoid"]};
