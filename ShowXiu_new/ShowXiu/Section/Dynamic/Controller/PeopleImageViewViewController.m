...
//  ShowXiu
//
//  Created by 上官昌璟 on 2017/4/19.
//  Copyright © 2017年 上官昌璟. All rights reserved.
//

#import "PeopleImageViewViewController.h"
...
@interface PeopleImageViewViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, assign)int HHKK;
...
@implementation PeopleImageViewViewController
...
- (void)dibuView{
    ...
    if ([uidS isEqualToString:@"257555"]) {
        ...
        liButton.titleLabel.font = [UIFont systemFontOfSize:18];
    ...
    guanButton.titleLabel.font = [UIFont systemFontOfSize:18];
    ...
    sixinButton.titleLabel.font = [UIFont systemFontOfSize:18];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        NSString *fensiST = [NSString stringWithFormat:@"%@粉丝",self.dataDic[@"fansnum"]];
        cell.shiyuLabel.text = fensiST;
        ...
        if (indexPath.row == 0) {
            ...
            if (Array.count ==  0) {
                if (self.LaeblLi) {
                    ...
                    _LaeblLi.font = [UIFont systemFontOfSize:14];
                    _LaeblLi.alpha = 0.3;
...
- (void)requestListData{
    ...
    NSUserDefaults *defatults = [NSUserDefaults standardUserDefaults];
    PublicAccordingModel *model = self.photoidArray[_HHKK];
    self.photoid = model.photoid;
    NSString *uidS = [defatults objectForKey:uidSG];
    NSString *URL = [NSString stringWithFormat:@"%@%@",AppURL,API_photohead];
    NSDictionary *dic = @{@"uid":uidS,@"photoid":model.photoid};
...
- (void)addgaunzhuButton:(UIButton *)sender{
    ...
    [HttpUtils postRequestWithURL:URL withParameters:dic withResult:^(id result) {
        ...
        [SVProgressHUD dismissWithDelay:1.0];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"adddengluShuaxin" object:nil];
        [self requestListData];
        [self.tabelView reloadData];
