...
@interface PotuViewController ()<UITableViewDelegate, UITableViewDataSource>
...
- (void)viewDidLoad {
    ...
    int ak = 0;
    for ( MianAllModel *model11 in self.photoidArray) {
        if ([model11.photoid isEqualToString:self.photoid]) {
            self.HHKK = ak;
        }
        ak = ak + 1;
    }
...
- (void)addtabelView{
    ...
    _tabelView.frame = CGRectMake(0, 0, kScreen_w, kScreen_h - 54);
...
- (void)addRibutton:(UIButton *)sender{
    _HHKK = _HHKK + 1;
    MianAllModel *model = self.photoidArray[_HHKK];
