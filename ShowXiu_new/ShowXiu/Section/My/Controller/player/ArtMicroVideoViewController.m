...
- (void)makeUI
{
    ...
    button.frame = CGRectMake(kScreen_w - 46, 40, 26, 23);
    [button setBackgroundImage:[UIImage imageNamed:@"phone"] forState:UIControlStateNormal];
    [button addTarget:self action:@selector(addFanButton:) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.recordBtnView.completeRecord = ^(CFTimeInterval recordTime){
        ...
        if (recordTime < 3.) {
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"录制时间太短" message:nil preferredStyle:UIAlertControllerStyleAlert];
...
- (void)addFanButton:(UIButton *)sender{
    if ([self.strTYpe isEqualToString:@"111"]) {
        self.strTYpe = @"222";
        [self setupVideo];
    }else {
        self.strTYpe = @"111";
