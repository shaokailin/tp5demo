...
+ (NSUInteger)httpReuquestWithEntity:(LSKParamterEntity *)entity success:(HttpSuccessBlock)success failure:(HttpFailureBlock)failure {
    ...
    if (entity) {
        switch (entity.requestType) {
            case 0:
            {
                identifier = [[LSKHttpManager sharedLSKHttpManager]_getReuqestWithApiPath:entity.requestApi params:entity.param success:success failure:failure];
                break;
            }
            case 2:
            {
                LSKMediaParamterEntity *mediaEntity = (LSKMediaParamterEntity *)entity;
                identifier =  [[LSKHttpManager sharedLSKHttpManager]_uploadImageReuqestWithApiPath:mediaEntity.requestApi params:mediaEntity.param name:mediaEntity.name type:mediaEntity.uploadType medias:mediaEntity.mediaArray success:success failure:failure];
                break;
            }
            case 1:
            default:
                identifier =  [[LSKHttpManager sharedLSKHttpManager]_postReuqestWithApiPath:entity.requestApi params:entity.param success:success failure:failure];
                break;
        }
    }
    return identifier;
