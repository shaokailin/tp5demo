...
- (void)initializeMainView:(NSString *)cancle {
    ...
    if (_otherBtnsArray && _otherBtnsArray.count > 0) {
        for (int i = 1; i <= _otherBtnsArray.count; i++) {
            ...
            UIButton *funcBtn = [LSKViewFactory initializeButtonWithTitle:title nornalImage:nil selectedImage:nil target:self action:@selector(actionSheetClick:) textfont:15 textColor:ColorUtilsString(kActionSheetText_Color) backgroundColor:[UIColor whiteColor] backgroundImage:nil];
            funcBtn.tag = 200 + i;
            ...
            if (i != _otherBtnsArray.count) {
                ...
                lineView.backgroundColor = ColorUtilsString(kActionSheetLine_Color);
    ...
    if ([cancle isHasValue]) {
        ...
        UIButton *cancleBtn = [LSKViewFactory initializeButtonWithTitle:cancle nornalImage:nil selectedImage:nil target:self action:@selector(actionSheetClick:) textfont:15 textColor:ColorUtilsString(kActionSheetText_Color) backgroundColor:[UIColor whiteColor] backgroundImage:nil];
        cancleBtn.tag = 200;
        cancleBtn.frame = CGRectMake(0, height, SCREEN_WIDTH, 44);
        [_contentView addSubview:cancleBtn];
        height += 44;
    }
    _bgView.frame = CGRectMake(0, 0, SCREEN_WIDTH, CGRectGetHeight(self.bounds) - height);
