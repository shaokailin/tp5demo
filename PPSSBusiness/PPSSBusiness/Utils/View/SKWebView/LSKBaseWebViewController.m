...
- (void)customMainView {
    ...
    [self addRightNavigationButtonWithNornalImage:@"webviewmore" seletedIamge:@"webviewmore" target:self action:@selector(showMoreAction)];
}
- (void)showMoreAction {
    @weakify(self)
    LSKActionSheetView *actionSheetView = [[LSKActionSheetView alloc]initWithCancelButtonTitle:@"取消" clcikIndex:^(NSInteger seletedIndex) {
        if (seletedIndex == 1) {
            @strongify(self)
           [self.webView reload];
        }
    } otherButtonTitles:@"刷新", nil];
    [actionSheetView showInView];
}
#pragma mark 加载导航栏
