...
- (void)setupContentView {
    ...
    if (_imageUrlArray.count < 2) {
        ...
        if (_imageUrlArray.count > 0) {
            imageView.imageWebUrl = [_imageUrlArray objectAtIndex:0];
        }
    ...
    for (int i = 0; i < imageArrayCount; i++) {
        ...
        if (i == 0) {
            imageView.imageWebUrl = _imageUrlArray.lastObject;
        }else if (i == imageArrayCount - 1){
            imageView.imageWebUrl = _imageUrlArray.firstObject;
        }else {
            imageView.imageWebUrl = [_imageUrlArray objectAtIndex:i - 1];
        }
...
- (NSTimer *)timer {
    if (!_timer) {
        @weakify(self)
        _timer = [NSTimer lsk_scheduledTimerWithTimeInterval:kFGGScrollInterval block:^(NSTimer *timer) {
            @strongify(self)
