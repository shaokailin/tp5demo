...
- (void)setupMoneyViewWithBalance:(NSString *)balance share:(NSString *)share {
    self.balanceLbl.text = NSStringFormat(@"￥%@",balance);
...
- (UIView *)_layoutMoneyView {
    ...
    [balanceView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.equalTo(moneyView.mas_centerX);
    }];
    return moneyView;
}
...
- (UIView *)_layoutInputView {
    ...
    ViewBorderLayer(textField, ColorHexadecimal(Color_APP_MAIN, 1.0), LINEVIEW_WIDTH / 2.0);
    ...
    UILabel *remarkLbl = [PPSSPublicViewManager initLblForColor9999:@"提示信息:满10元可提现"];
