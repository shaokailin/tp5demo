...
- (instancetype)initWithEventBlock:(OrderHomeScreenBlock)clickBlock tabbarHeight:(CGFloat)height {
    if (self = [super init]) {
        _payStateIndex = -1;
...
- (void)_layoutContentView {
    ...
    UIButton *alipayBtn = [self customBtnWithTitle:@"支付宝" tag:203];
    [_contentView addSubview:alipayBtn];
    
    UIButton *wxBtn = [self customBtnWithTitle:@"微信" tag:204];
    [_contentView addSubview:wxBtn];
    
    UIButton *chuBtn = [self customBtnWithTitle:@"本店余额" tag:202];
    [_contentView addSubview:chuBtn];
    
    UIButton *yuBtn = [self customBtnWithTitle:@"通用余额" tag:201];
    ...
    UIButton *successBtn = [self customBtnWithTitle:@"支付成功" tag:301];
    [_contentView addSubview:successBtn];
    
    UIButton *errorBtn = [self customBtnWithTitle:@"未支付" tag:302];
    ...
    UIButton *resetBtn = [LSKViewFactory initializeButtonWithTitle:@"重置" nornalImage:nil selectedImage:nil target:self action:@selector(resetClickEvent) textfont:18 textColor:_appColor backgroundColor:nil backgroundImage:nil];
    ...
    UIButton *sureBtn = [PPSSPublicViewManager initAPPThemeBtn:@"确定" font:18 target:self action:@selector(sureSelectEvent)];
...
- (UIButton *)customBtnWithTitle:(NSString *)title tag:(NSInteger)tag {
    ...
    UIButton *btn = [LSKViewFactory initializeButtonWithTitle:title nornalImage:nil selectedImage:nil target:self action:@selector(selectBtnClick:) textfont:12 textColor:color backgroundColor:nil backgroundImage:nil];
    btn.tag = tag;
    [btn setTitleColor:COLOR_WHITECOLOR forState:UIControlStateSelected];
    ViewRadius(btn, 3);
    ViewBorderLayer(btn, ColorHexadecimal(Color_Text_9999, 1.0), LINEVIEW_WIDTH / 2.0);
    return btn;
}
...
- (void)cancleSelect:(NSInteger)tag {
    ...
    otherBtn.layer.borderWidth = LINEVIEW_WIDTH / 2.0;
...
- (void)selectBtnClick:(UIButton *)btn {
    if (!btn.selected) {
        ...
        if (btn.tag < 300) {
            if (_payTypeSelectIndex != -1) {
                otherBtn = [_contentView viewWithTag:_payTypeSelectIndex + 200];
            }
            _payTypeSelectIndex = btn.tag - 200;
        }else {
            if (_payStateIndex != -1) {
                otherBtn = [_contentView viewWithTag:_payStateIndex + 300];
            }
            _payStateIndex = btn.tag - 300;
        }
        btn.selected = YES;
