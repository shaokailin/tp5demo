...
- (void)keyboardClick:(UIButton *)btn {
    ...
    if (tag <= 9) {
        ...
        [self.inputString appendFormat:@"%zd",tag];
        self.inputStr = self.inputString;
    }else {
        if (tag == 10) {
            NSRange range = [self.inputString rangeOfString:@"0"];
            if (range.location == 0 && self.inputString.length == 1) {
                return;
            }
...
//            }
        ...
        if (_currentOperation != btn.tag - 212) {
...
- (NSString *)numberOperator {
    BOOL isBefore = KJudgeIsNullData(self.operatorBeforeString);
    BOOL isCurrent = KJudgeIsNullData(self.inputString);
    NSString *result = nil;
    if (isBefore && !isCurrent) {
        result = self.operatorBeforeString;
    }
    if (!isBefore && isCurrent) {
         result = [self.inputString copy];
    }
    if (isBefore && isCurrent) {
...
- (void)_layoutMainView {
    ...
    [_collectBtn setBackgroundImage:ImageNameInit(@"shoukuan_ico_hui") forState:UIControlStateSelected];
    [_collectBtn setBackgroundImage:ImageNameInit(@"shoukuankuang_ico") forState:UIControlStateNormal];
    _collectBtn.titleLabel.numberOfLines = 0;
    _collectBtn.titleEdgeInsets = UIEdgeInsetsMake(0, (_btnWidth - font) / 2, 0, (_btnWidth - font) / 2);
    _collectBtn.backgroundColor = nil;
...
- (UIButton *)customKeyboardImgBtnWithImg:(NSString *)img tag:(NSInteger)tag {
    UIButton *btn = [LSKViewFactory initializeButtonWithTitle:nil nornalImage:img selectedImage:nil target:self action:@selector(keyboardOperatorClick:) textfont:WIDTH_RACE_6S(29) textColor:ColorHexadecimal(Color_Text_3333, 1.0) backgroundColor:nil backgroundImage:@"shuzikuang_ico"];
    [btn setBackgroundImage:ImageNameInit(@"shuzi_bj_ico") forState:UIControlStateSelected];
    ViewRadius(btn, 3.0);
    ViewBorderLayer(btn, _nornalColor, LINEVIEW_WIDTH / 2.0);
    btn.tag = 200 + tag;
    return btn;
}
...
- (UIButton *)customKeyboardBtnWithIndex:(NSInteger)index {
    ...
    if (index == 11) {
        ...
        title = NSStringFormat(@"%zd",index == 10? 0:index);
    }
    UIButton *btn = [LSKViewFactory initializeButtonWithTitle:title nornalImage:nil selectedImage:nil target:self action:@selector(keyboardClick:) textfont:WIDTH_RACE_6S(29) textColor:ColorHexadecimal(Color_Text_3333, 1.0) backgroundColor:nil backgroundImage:@"shuzikuang_ico"];
    [btn setBackgroundImage:ImageNameInit(@"shuzi_bj_ico") forState:UIControlStateSelected];
    ViewRadius(btn, 3.0);
    ViewBorderLayer(btn, _nornalColor, LINEVIEW_WIDTH / 2.0);
    btn.tag = 200 + index;
