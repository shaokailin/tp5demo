...
- (void)setMaxDate:(NSDate *)maxDate {
    [_pickView setMaxDate:maxDate];
}
- (void)setMinDate:(NSDate *)minDate {
    [_pickView setMinDate:minDate];
}
#pragma mark -view
- (void)initializeMainView {
    self.isAutoHidenForSure = YES;
    self.hidden = YES;
...
- (void)_latoutUnderButtonView {
    ...
    UIButton *cancleBtn = [LSKViewFactory initializeButtonWithTitle:@"取消" nornalImage:nil selectedImage:nil target:self action:@selector(cancleSelectedClick) textfont:18 textColor:ColorHexadecimal(Color_Text_6666, 1.0) backgroundColor:nil backgroundImage:nil];
    ...
    UIButton *sureBtn = [LSKViewFactory initializeButtonWithTitle:@"确定" nornalImage:nil selectedImage:nil target:self action:@selector(sureSelectClick) textfont:18 textColor:COLOR_WHITECOLOR backgroundColor:ColorHexadecimal(Color_APP_MAIN, 1.0) backgroundImage:nil];
...
- (void)showInView {
    if (!_isShow) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            _bgView.alpha = 1;
            _contentView.frame = CGRectMake(0, CGRectGetHeight(self.frame) - kContentViewHeight - _tabbarHeight , SCREEN_WIDTH, kContentViewHeight);
        }];
    }
}
