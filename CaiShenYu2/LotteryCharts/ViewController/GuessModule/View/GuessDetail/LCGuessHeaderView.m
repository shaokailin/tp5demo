...
- (void)setupContentTitle:(NSString *)title money:(NSString *)money count:(NSInteger)count number1:(NSString *)number1 number2:(NSString *)number2 type:(NSInteger)type model:(LCGuessModel *)model  {
    if (type == 0) {
        ...
        if ([model.quiz_answer isEqualToString:@"小"]) {
            [self.sizeBtn setTitle:@"大" forState:UIControlStateNormal];
        } else {
            [self.sizeBtn setTitle:@"小" forState:UIControlStateNormal];
        }
    ...
    [self.canyuBtn setTitle:[NSString stringWithFormat:@"%@人参与 >", model.challenge_count] forState:UIControlStateNormal];
}
...
- (IBAction)canyuAction:(UIButton *)sender {
    if (self.myBlock) {
        self.myBlock(sender);
    }
}
