...
- (void)bindSignal {
    ...
    _viewModel = [[LCTeamViewModel alloc]initWithSuccessBlock:^(NSUInteger identifier, id model) {
        ...
        if (identifier == 0) {
            ...
            [self.headerView setupContentWithLineCount:dic[@"sign_ymoney"] allCount:dic[@"sign_count"]];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell setupContentWithPhoto:model.logo name:model.nickname userId:model.mch_no glodCount:model.money yinbiCount:model.ymoney type:_showType state:_showType == 0? model.is_online : model.is_sign];
    return cell;
}
