...
- (void)bindSignal {
    ...
    _viewModel = [[LCHomeMainViewModel alloc]initWithSuccessBlock:^(NSUInteger identifier, id model) {
        ...
        if (identifier == 0) {
            ...
            postModel.fans_count = detaiModel.fans_count;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell setupContentWithPhoto:model.logo name:model.nickname userId:model.mch_no postId:model.post_id time:model.create_time title:model.post_title showCount:model.thumbsup_count money:model.post_money funs:KNullTransformNumber(model.fans_count)comtent:model.reply_count];
    WS(ws)
