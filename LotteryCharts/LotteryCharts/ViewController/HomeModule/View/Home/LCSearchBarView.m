...
- (void)_layoutMainView {
    ...
    UIButton *btn = [LSKViewFactory initializeButtonWithTitle:@"搜索" target:self action:@selector(showSearchClick) textfont:15 textColor:ColorHexadecimal(0x434343, 1.0)];
    [self addSubview:btn];
...
- (void)showSearchMeun:(UIButton *)sender {
    [_searchField resignFirstResponder];
    if (self.searchBlock) {
        self.searchBlock(1,sender);
    }
}
- (void)actionClickEvent:(NSInteger)type {
    [_searchField resignFirstResponder];
    if (self.searchBlock) {
        self.searchBlock(type,self.searchText);
    }
}
- (NSString *)searchText {
